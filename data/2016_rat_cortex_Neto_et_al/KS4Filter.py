...
def compute_preprocessing(ops, device, tic0=np.nan, file_object=None):
    ...
    # changing to local
    bfile = BinaryFiltered(ops['filename'], n_chan_bin, fs, NT, nt, twav_min,
...
def compute_drift_correction(ops, device, tic0=np.nan, progress_bar=None,
    ...
    ops : dict
        Dictionary storing settings and results for all algorithmic steps.
    bfile : BinaryFiltered **local copy
...
class BinaryFiltered(io.BinaryRWFile):
    ...
    def filter(self, X, ops=None, ibatch=None, skip_preproc=False):
        ...
        if self.hp_filter is not None:
            ...
            # computing window size from frame rate
            winSize = int(self.fs/1000 + 1)

            X_nd = X.cpu()

            for chan in self.chan_map:
                LFiltered = uniform_filter1d(X_nd[chan], size=winSize, mode='mirror')
                HFiltered = X_nd[chan] - LFiltered
                X_nd[chan] = HFiltered

            X = X_nd.to(self.device)
        ...
        if self.whiten_mat is not None:
            if self.dshift is not None and ops is not None and ibatch is not None:
                M = io.get_drift_matrix(ops, self.dshift[ibatch], device=self.device)
